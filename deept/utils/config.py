...
class Config(CoolConfig):

    @staticmethod
    def parse_config_from_args(args, path=None):
        config = CoolConfig.parse_config_from_args(args, path=path)
        config.register_custom_get_item_hook(Config.assert_check_item)
        return config

    @staticmethod
    def parse_config_from_path(path):
        config = CoolConfig.parse_config_from_path(path)
        config.register_custom_get_item_hook(Config.assert_check_item)
        return config

    @staticmethod
    def parse_config_from_dict(raw_config, root_dir):
        config = CoolConfig.parse_config_from_dict(raw_config, root_dir)
        config.register_custom_get_item_hook(Config.assert_check_item)
        return config

    @staticmethod
    def assert_check_item(key, item):
